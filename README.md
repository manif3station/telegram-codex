...
Task-style requests such as `run`, `check`, `review`, `verify`, `test`, `finish`, or `fix` now open that verbose trace immediately with a kickoff line before richer Codex JSON events arrive.
